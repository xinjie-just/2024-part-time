...
import { IManageConfig, IRouterType } from '@/models';
import { routes } from '@/routers';
import { getConfigInfo, saveConfigInfo } from '@/services';
import { ISaveConfigInfoReq } from '@/services/models';
...
const props = defineProps<{ isEdit: boolean; ConfigId: number }>();

const visible = ref(true);
const form: UnwrapRef<IManageConfig> = reactive({
  name: '',
  loginName: '',
  phone: '',
  password: ''
...
const getConfigInfoFn = () => {
  const params = {
    adminId: props.ConfigId,
  };
...
const onSubmit = async (): Promise<void> => {
  ...
  try {
    ...
    const params: ISaveConfigInfoReq = {
      name: form.name,
